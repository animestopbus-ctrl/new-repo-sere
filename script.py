...
import httpx  # ← Async & faster than requests (latest 2026 best practice)
...
from telegram import (
    ...
    InlineKeyboardMarkup, InputMediaPhoto, WebAppInfo
)
...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[logging.StreamHandler(), logging.FileHandler("renamer.log", encoding="utf-8")]
)
logger = logging.getLogger(__name__)

# 🔥 DYNAMIC DOMAIN ENGINE (unchanged but safer)
DOMAIN = os.getenv("RENDER_EXTERNAL_URL", os.getenv("WEB_URL", "https://new-repo-sere.onrender.com")).rstrip('/')

# 🛡️ ANTI-SPAM + METADATA CACHE (Memory + TTL)
SPAM_CACHE: dict[int, float] = {}
METADATA_CACHE: dict[str, dict] = {}
CACHE_TTL = 3600  # 1 hour

# ================= GLOBAL TEXT CONSTANTS (Updated to Bot API 9.4 - Feb 2026) =================
...
<blockquote>🟢 <b>Version:</b> v2.2.0 (Bot API 9.4 — Feb 2026)
👨‍💻 <b>Developer:</b> <a href="https://t.me/DmOwner">@Ⓜ️ark</a>
🐍 <b>Language:</b> Python 3.12+
🗄️ <b>Database:</b> MongoDB
🚀 <b>Features:</b> Colored buttons + Web Apps + Async engine</blockquote>"""

# ================= UTILITIES (Improved + Async) =================
async def get_img():
...
def esc(text: str | None) -> str:
    if not text or str(text).lower() in {'none', 'nan', 'null', 'n/a'}:
        return "N/A"
...
def pre_clean_filename(filename: str) -> str:
    ...
    f = re.sub(r'(?i)DA Rips|HEVC|WEB-DL|X264|X265|1080p|720p|480p', '', f, flags=re.I)
    f = re.sub(r'\[.*?\]|\(.*?\)', '', f)
    f = re.sub(r'[\.\_]+', ' ', f)
...
def detect_languages(filename: str, guessit_langs) -> str:
    found = []
    fname_lower = filename.lower()

    if guessit_langs:
        if not isinstance(guessit_langs, list):
            guessit_langs = [guessit_langs]
        for l in guessit_langs:
            lang_str = str(l).lower()
            found.append(secret.LANG_MAP.get(lang_str, lang_str.capitalize()))

    if 'dual' in fname_lower:
        found.append('Dual Audio')
    if 'multi' in fname_lower:
        found.append('Multi Audio')
    if 'hin' in fname_lower and 'Hindi' not in found:
        found.append('Hindi')
    if 'tam' in fname_lower and 'Tamil' not in found:
        found.append('Tamil')
    if 'tel' in fname_lower and 'Telugu' not in found:
        found.append('Telugu')
    if 'kor' in fname_lower and 'Korean' not in found:
        found.append('Korean')
    if 'eng' in fname_lower and 'English' not in found:
...
async def get_real_resolution(file_id: str, context: ContextTypes.DEFAULT_TYPE) -> str | None:
    try:
        tg_file = await context.bot.get_file(file_id)
        file_url = f"https://api.telegram.org/file/bot{secret.BOT_TOKEN}/{tg_file.file_path}"

        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(file_url, headers={"Range": "bytes=0-1048576"})
            chunk = resp.content
        ...
        if parser:
            with parser:
                ...
                if meta:
                    ...
                    if w and h:
                        if w >= 3800 or h >= 2100:
                            res_display = "4K (2160p)"
                        elif w >= 2500 or h >= 1400:
                            res_display = "2K (1440p)"
                        elif w >= 1900 or h >= 1000:
                            res_display = "FHD (1080p)"
                        elif w >= 1200 or h >= 700:
                            res_display = "HD (720p)"
                        elif w >= 800 or h >= 480:
                            res_display = "SD (480p)"
                        else:
                            res_display = f"{w}x{h}p"
        return res_display
    except Exception as e:
        logger.warning(f"Resolution detection failed for {file_id}: {e}")
        return None
    finally:
        try:
            os.unlink(tmp_path)
...
async def fetch_smart_metadata(title: str, year: int | None, original_filename: str) -> dict:
    """Async + Cached (production ready)"""
    cache_key = f"{title}:{year or ''}"
    if cache_key in METADATA_CACHE:
        return METADATA_CACHE[cache_key]

    data = {"title": title, "rating": "N/A", "genres": "Misc", "date": "N/A", "type": "movie"}
    is_anime_hint = any(x in original_filename.lower() for x in ['anime', 'judas', 'subs'])

    async with httpx.AsyncClient(timeout=8.0) as client:
        try:
            # TMDB
            tm_key = random.choice(secret.TMDB_KEYS)
            url = f"https://api.themoviedb.org/3/search/multi?api_key={tm_key}&query={httpx.quote(title)}"
            ...
            if res.get('results'):
                ...
                if genre_list:
                    data['genres'] = ", ".join(genre_list[:3])

                country = best_item.get('origin_country', [''])[0]
                ...
                if is_animation and (country == 'JP' or language == 'ja'):
                    ...
                    if country == 'KR' or language == 'ko':
                        data['type'] = 'kdrama'
                    elif country == 'CN' or language == 'zh':
                        data['type'] = 'cdrama'
                    elif country == 'JP' or language == 'ja':
                        data['type'] = 'jdrama'
                elif data['type'] == 'movie':
                    if country == 'IN' or language in ['hi', 'ta', 'te', 'ml']:
                        data['type'] = 'indian'
                    elif country == 'KR' or language == 'ko':
                        data['type'] = 'kmovie'
                    elif country == 'JP' or language == 'ja':
                        data['type'] = 'jmovie'
        ...
        if data['rating'] == 'N/A' and data['type'] in ['series', 'kdrama', 'cdrama', 'jdrama']:
            try:
                res = (await client.get(f"https://api.tvmaze.com/singlesearch/shows?q={httpx.quote(title)}")).json()
                if res:
                    data['title'] = res.get('name', data['title'])
                    if res.get('rating', {}).get('average'):
                        data['rating'] = f"{res['rating']['average']} ⭐"
                    data['date'] = res.get('premiered', data['date'])[:4] if res.get('premiered') else data['date']
                    if res.get('genres'):
                        data['genres'] = ", ".join(res['genres'][:3])
            except:
                pass
        ...
        if data['type'] == 'anime' or is_anime_hint:
            try:
                res = (await client.get(f"https://api.jikan.moe/v4/anime?q={httpx.quote(title)}&limit=1")).json()
                if res.get('data'):
                    anime = res['data'][0]
...
# ================= KEYBOARDS (Bot API 9.4 colored buttons preserved) =================
def get_main_menu_markup():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📢 JOIN OFFICIAL CHANNEL", url="https://t.me/THEUPDATEDGUYS", api_kwargs={"style": "primary"})],
        [
            InlineKeyboardButton("📚 How to Use", callback_data="help_menu", api_kwargs={"style": "success"}),
            InlineKeyboardButton("⚙️ Settings", callback_data="settings_menu", api_kwargs={"style": "primary"})
        ],
...
def get_media_markup(title: str, is_generated: bool = False):
    imdb_url = f"https://www.imdb.com/find/?q={httpx.quote(title.replace(' ', '+'))}"
    buttons = []
    if not is_generated:
        buttons.append([InlineKeyboardButton("🔗 Generate Direct Links", callback_data="ask_timer", api_kwargs={"style": "primary"})])
    else:
        buttons.append([InlineKeyboardButton("✅ Links Generated Below", callback_data="ignore", api_kwargs={"style": "success"})])
    buttons.append([
        InlineKeyboardButton("🎬 IMDB INFO", url=imdb_url, api_kwargs={"style": "danger"}),
...
def get_timer_markup():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🕒 1 Hour", callback_data="timer_1", api_kwargs={"style": "primary"}),
            InlineKeyboardButton("🕒 6 Hours", callback_data="timer_6", api_kwargs={"style": "primary"})
        ],
        [
            InlineKeyboardButton("🕒 12 Hours", callback_data="timer_12", api_kwargs={"style": "primary"}),
            InlineKeyboardButton("🕒 24 Hours", callback_data="timer_24", api_kwargs={"style": "danger"})
        ],
        [InlineKeyboardButton("⬅️ Cancel", callback_data="cancel_timer", api_kwargs={"style": "danger"})]
    ])


def get_url_markup(hash_id: str):
...
async def send_recon_log(user, context: ContextTypes.DEFAULT_TYPE):
    if not secret.LOG_CHANNEL_ID:
        return
    username_fmt = f"@{user.username}" if user.username else "N/A"
    last_name = user.last_name or "N/A"
    log_text = f"🆕 <b>NEW USER DETECTED</b>\n\n<blockquote>👤 <b>First Name:</b> {esc(user.first_name)}\n🗣 <b>Last Name:</b> {esc(last_name)}\n🔗 <b>Username:</b> {esc(username_fmt)}\n🆔 <b>User ID:</b> <code>{user.id}</code></blockquote>"
    try:
        photos = await context.bot.get_user_profile_photos(user.id)
        if photos.total_count > 0:
            await context.bot.send_photo(chat_id=secret.LOG_CHANNEL_ID, photo=photos.photos[0][-1].file_id,
                                         caption=log_text, parse_mode=ParseMode.HTML, disable_notification=True)
        else:
            await context.bot.send_message(chat_id=secret.LOG_CHANNEL_ID, text=log_text,
...
async def ping_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await update.message.set_reaction(reaction=ReactionTypeEmoji(random.choice(secret.EMOJIS)), is_big=True)
    except:
        pass
    start_t = time.time()
    msg = await update.message.reply_text("📶 Pinging Server...", parse_mode=ParseMode.HTML)
    await msg.edit_text(f"🏓 <b>Pong!</b>\n<blockquote>Latency: <code>{round((time.time() - start_t) * 1000)}ms</code></blockquote>",
...
async def alive_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.set_reaction(reaction=ReactionTypeEmoji(random.choice(secret.EMOJIS)), is_big=True)
        await update.message.reply_sticker(sticker=random.choice(secret.LOADING_STICKERS))
    except:
        pass
    await safe_reply(update.message, "<b>Yes darling, I am alive. Don't worry! 😘</b>",
                     parse_mode=ParseMode.HTML, message_effect_id=random.choice(secret.MESSAGE_EFFECTS))
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await safe_reply(update.message, "<b>🚀 Send me any Movie, Series, or Anime file and I will process it instantly!</b>",
                     parse_mode=ParseMode.HTML, message_effect_id=random.choice(secret.MESSAGE_EFFECTS))


# ================= START / HELP / INFO / SETTINGS =================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await db.get_maintenance() and user.id != secret.ADMIN_ID:
        return await update.message.reply_text("🚧 <b>MAINTENANCE MODE</b>\n\n<blockquote>The bot is currently undergoing upgrades. Please try again later.</blockquote>",
                                               parse_mode=ParseMode.HTML)
    ...
    if not await fsub.is_user_subscribed(context.bot, user.id):
        ...
        try:
            await sent_msg.set_reaction(reaction=ReactionTypeEmoji("🛑"), is_big=True)
        except:
            pass
        return
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        pass
    img = await get_img()
    sent_msg = await update.message.reply_photo(photo=img, caption=HELP_TEXT, parse_mode=ParseMode.HTML,
...
async def info_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        pass
    markup = InlineKeyboardMarkup([[InlineKeyboardButton("👨‍💻 Contact Dev", url="https://t.me/DmOwner",
                                                         api_kwargs={"style": "primary"})]])
    img = await get_img()
    sent_msg = await update.message.reply_photo(photo=img, caption=INFO_TEXT, parse_mode=ParseMode.HTML, reply_markup=markup)
    try:
        await sent_msg.set_reaction(reaction=ReactionTypeEmoji("ℹ️"), is_big=True)
    except:
        pass
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_data:
        return await update.message.reply_text("❌ Please send /start first to register your account.")
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("💎 Buy Premium", url="https://t.me/DmOwner", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("⬅️ Back", callback_data="main_menu", api_kwargs={"style": "danger"})]
    ])
    img = await get_img()
    sent_msg = await update.message.reply_photo(photo=img, caption=text, parse_mode=ParseMode.HTML, reply_markup=markup)
    try:
        await sent_msg.set_reaction(reaction=ReactionTypeEmoji("⚙️"), is_big=True)
    except:
        pass
...
async def feedback_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not feedback_text:
        return await update.message.reply_text("❌ <b>Format:</b> <code>/feedback [Type your message here]</code>\n\n<i>Example: /feedback The bot isn't catching Hindi language correctly.</i>",
                                               parse_mode=ParseMode.HTML)
    ...
    try:
        ...
        await safe_reply(update.message, "✅ <b>Feedback Sent Successfully!</b>\n<blockquote>Thank you for helping us improve the engine.</blockquote>",
                         parse_mode=ParseMode.HTML, message_effect_id=random.choice(secret.MESSAGE_EFFECTS))
    except Exception:
        await update.message.reply_text("❌ Failed to send feedback to the developer.", parse_mode=ParseMode.HTML)
...
async def set_cap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await db.check_premium_status(user_id):
        return await update.message.reply_text("💎 <b>PREMIUM FEATURE:</b>\n<blockquote>You must be a Premium user to set custom captions!</blockquote>",
                                               parse_mode=ParseMode.HTML)
    ...
    await safe_reply(update.message, "✅ <b>SUCCESS:</b> Custom caption saved!\n<blockquote>It will now appear at the bottom of your files.</blockquote>",
                     parse_mode=ParseMode.HTML, message_effect_id=random.choice(secret.MESSAGE_EFFECTS))
...
async def del_cap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_reply(update.message, "🗑️ Custom caption removed. Reverted to default.",
                     parse_mode=ParseMode.HTML, message_effect_id=random.choice(secret.MESSAGE_EFFECTS))
...
async def my_cap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cap:
        await safe_reply(update.message, f"📝 <b>Your Custom Caption:</b>\n\n<blockquote>{cap}</blockquote>",
                         parse_mode=ParseMode.HTML, message_effect_id=random.choice(secret.MESSAGE_EFFECTS))
    else:
        await update.message.reply_text("You have no custom caption set. Using default.", parse_mode=ParseMode.HTML)
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        if await db.is_banned(user.id):
            return await msg.reply_text("🔨 <b>ACCESS DENIED:</b> You are permanently banned.", parse_mode=ParseMode.HTML)
        if await db.check_limit(user.id):
            return await msg.reply_text("⚠️ <b>DAILY LIMIT REACHED!</b>\n<blockquote>You used your 10 free renames today.\n<i>Upgrade to Premium for unlimited!</i></blockquote>",
                                        parse_mode=ParseMode.HTML)
    ...
    info = await fetch_smart_metadata(search_q, search_year, original_name)
    ...
    if query:
        ...
        try:
            await context.bot.set_message_reaction(
                ...
                reaction=ReactionTypeEmoji(random.choice(secret.EMOJIS)),
                is_big=True
            )
        except:
            pass
...
def get_title_from_caption(caption: str | None) -> str:
    if not caption:
        return "Unknown"
...
# ================= CALLBACK ROUTER (Improved + Anti-Spam) =================
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Anti-spam (only on timer actions)
    ...
    if data == "ask_timer":
        ...
        if not media:
            return await query.answer("❌ No file detected.", show_alert=True)
        ...
        await safe_reply(query.message, text=link_text, parse_mode=ParseMode.HTML,
        ...
        try:
            await query.edit_message_media(
                media=InputMediaPhoto(media=img, caption=HELP_TEXT, parse_mode=ParseMode.HTML),
                reply_markup=get_help_menu_markup()
            )
        except BadRequest:
            pass
        ...
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("👨‍💻 Contact Dev", url="https://t.me/DmOwner", api_kwargs={"style": "primary"})],
            [InlineKeyboardButton("⬅️ Back", callback_data="main_menu", api_kwargs={"style": "danger"})]
        ])
        try:
            await query.edit_message_media(
                media=InputMediaPhoto(media=img, caption=INFO_TEXT, parse_mode=ParseMode.HTML),
                reply_markup=markup
            )
        except BadRequest:
            pass
        ...
        try:
            await query.edit_message_media(
                media=InputMediaPhoto(media=img, caption=START_TEXT.format(name=esc(update.effective_user.first_name)),
                                      parse_mode=ParseMode.HTML),
                reply_markup=get_main_menu_markup()
            )
        except BadRequest:
...
# ================= AUTO CACHE CLEAN (call this from your main file if needed) =================
async def clean_cache():
    """Call this periodically from your job queue"""
    now = time.time()
    for k in list(SPAM_CACHE.keys()):
        if now - SPAM_CACHE[k] > 300:
            del SPAM_CACHE[k]
    for k in list(METADATA_CACHE.keys()):
        # TTL handled by replacement, but you can add timestamp if you want
        pass
